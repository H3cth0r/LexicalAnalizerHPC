...
bool append_st_map(std::string t_lexeme, SymbolInfo si){
    SymbolsTable.insert({t_lexeme, si});
    return true;
}
...
/*
    Here I'm binding the whole code with PYbind11.
    In this case, I'm Actually bindint the whole code, it was not necessary;
    i could have just bind the Lexer method, but this way I've have created 
    a library which let us create token, symbol tables, Symbol info and 
    make use of the lexer. 
*/
namespace py = pybind11;
PYBIND11_MODULE(hpcLexAnalyzer, m){
    m.doc() = "HPC language Lexical Analyzer.";

    py::bind_map<std::unordered_map<std::string, std::string>>(m, "keywords_tokens");

    py::class_<SymbolInfo>(m, "SymbolInfo").def(py::init<std::string, std::string, bool, int>())
        .def("get_lexeme",              &SymbolInfo::get_lexeme)
        .def("get_tokenKind",           &SymbolInfo::get_tokenKind)
        .def("get_declared",            &SymbolInfo::get_declared)
        .def("get_address",             &SymbolInfo::get_address)
        .def("set_lexeme",              &SymbolInfo::set_lexeme)
        .def("set_tokenKind",           &SymbolInfo::set_tokenKind)
        .def("set_declared",            &SymbolInfo::set_declared)
        .def("set_address",             &SymbolInfo::set_address)
        .def_property("lexeme",         &SymbolInfo::get_lexeme,        &SymbolInfo::set_lexeme)
        .def_property("tokenKind",      &SymbolInfo::get_tokenKind,     &SymbolInfo::set_tokenKind)
        .def_property("declared",       &SymbolInfo::get_declared,      &SymbolInfo::set_declared)
        .def_property("address",        &SymbolInfo::get_address,       &SymbolInfo::set_address)
        .def("print",                   &SymbolInfo::print);
    
    py::bind_map<std::map<std::string, SymbolInfo>>(m, "SymbolsTable");
    m.def("append_st_map", &append_st_map, "Append to st_map");

    py::class_<Token>(m, "Token").def(py::init<std::string, std::string, int, int, std::string, int>())
        .def(py::init<>())
        //      GETTERS
        .def("get_kind",                &Token::get_kind)
        .def("get_lexene",              &Token::get_lexeme)
        .def("get_line",                &Token::get_line)
        .def("get_character_index",     &Token::get_character_index)
        .def("get_index",               &Token::get_index)
        .def("get_color",               &Token::get_color)
        .def("get_token_size",          &Token::get_token_size)
        //    SETTERS
        .def("set_kind",                &Token::set_kind)
        .def("set_lexene",              &Token::set_lexeme)
        .def("set_line",                &Token::set_line)
        .def("set_character_index",     &Token::set_character_index)
        .def("set_index",               &Token::set_index)
        .def("set_color",               &Token::set_color)
        .def("set_token_size",          &Token::set_token_size)
        //      FUNTIONALITY METHODS
        .def("print",                   &Token::print)
        //      DEF PROTERTY
        .def_property("kind",           &Token::get_kind,               &Token::set_kind)
        .def_property("lexeme",         &Token::get_lexeme,             &Token::set_lexeme)
        .def_property("line",           &Token::get_line,               &Token::set_line)
        .def_property("character_index",&Token::get_character_index,    &Token::set_character_index)
        .def_property("index",          &Token::get_index,              &Token::set_index)
        .def_property("color",          &Token::get_color,              &Token::set_color)
        .def_property("token_size",     &Token::get_token_size,         &Token::set_token_size);

    py::class_<Lexer>(m, "Lexer").def(py::init<std::string>())
        .def(py::init<>())
        //      GETTERS
        .def("get_character",           &Lexer::get_character)
        .def("get_input",               &Lexer::get_input)
        //      SETTERS
        .def("set_character",           &Lexer::set_character)
        .def("set_input",               &Lexer::set_input)
        //      FUNCTIONALITY METHODS
        .def("isDigit",                 &Lexer::isDigit)
        .def("isLetter",                &Lexer::isLetter)
        .def("isSeparator",             &Lexer::isSeparator)
        .def("consume",                 &Lexer::consume)
        .def("calculate_in_line_index", &Lexer::calculate_in_line_index)
        .def("tokenId",                 &Lexer::tokenId)
        .def("tokenValIntFloat",        &Lexer::tokenValIntFloat)
        .def("nextToken",               &Lexer::nextToken)
        .def("get_all_tokens",          &Lexer::get_all_tokens)
        .def("print_symbol_table",      &Lexer::print_symbol_table)
        .def("get_symbol_table",        &Lexer::get_symbol_table)
        //      DEF PROPERTY
        .def_property("character",      &Lexer::get_character,          &Lexer::set_character)
        .def_property("input",          &Lexer::get_input,              &Lexer::set_input)
        ;
